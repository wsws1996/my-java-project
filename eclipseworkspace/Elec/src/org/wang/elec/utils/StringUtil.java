...
public class StringUtil {
	 ...
	 */
	public static List<String> getContentByList(String wholecontent,
			int cutcount) {
		...
		// 内容长度超过截取长度
			...
			for (int i = 1; i <= contentround; i++) {
				...
				if (i != contentround) {
					// 按照截断长度截取内容
					contentpart = wholecontent.substring(begincount, cutcount
							* i);
				} else {
					// 截取最后一部分内容
					contentpart = wholecontent
							.substring(begincount, contentlen);
				}
				// 赋值下一截取部分的起点位置
