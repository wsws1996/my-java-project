...
//登陆
function login(){
	var h = "/shopping/login.shtml?returnUrl="+window.location.href;
			...
			<ul class="uls filter">
				...
				<li><b>品牌：</b><p>
					...
					<c:forEach items="${brands }" var="brand">
						<a href="javascript:void(0);" onclick="window.location.href='/product/display/list.shtml?brandId=${brand.id}&brandName=${brand.name }&typeId=${typeId }&typeName=${typeName }'" title="${brand.name }">${brand.name }</a>
					</c:forEach>
				</p></li>
				</c:if>
				<li><b>价格：</b><p>
